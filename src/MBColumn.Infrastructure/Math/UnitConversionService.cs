...
public sealed class UnitConversionService : IUnitConversionService
{
    ...
    public double ConvertForce(double value, ForceUnit fromUnit, ForceUnit toUnit) => ForceFromN(ForceToN(value, fromUnit), toUnit);
    ...
    {
        var valueNmm = ForceToN(value, fromForceUnit) * LengthToMm(1.0, fromLengthUnit);
        return valueNmm / (ForceToN(1.0, toForceUnit) * LengthToMm(1.0, toLengthUnit));
    }
