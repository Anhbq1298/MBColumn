...
public static class ControlPointTableBuilderService
{
    ...
    // theta=90 → compression in +y → bending about X axis → Mnx moment (spColumn "X")
    // theta=0  → compression in +x → bending about Y axis → Mny moment (spColumn "Y")
        ...
        IDesignCodeService code)
    {
        ...
        foreach (var (axisLabel, angleDeg) in Axes)
        {
            ...
            // Sorted descending by depth → descending c → descending P (compression → tension)
            ...
            [
                ...
                InterpolateAtPn(pts, 0.0),                            // 6 Pure bending — bracket on Pn (not PhiPn)
                pts[^1],                                               // 7 Max tension
            ];
            ...
            for (int k = 0; k < labeled.Length; k++)
            {
                ...
                // Max tension row uses the sentinel 9.99999 (spColumn convention for c → 0).
                double epsilonT;
                if (isMaxTension)
                ...
                // Pure bending: P is zero by definition — hardcode to eliminate floating-point
    ...
    /// Pn ≠ 0 at the interpolated point. Mirrors spColumn's invariant that
