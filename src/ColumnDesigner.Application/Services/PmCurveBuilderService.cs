...
public static class PmCurveBuilderService
{
    ...
    private static IReadOnlyList<ControlPointDto> BuildCurve(IEnumerable<ControlPoint> source, CurveAxis axis, double designCompressionLimitDisplay)
    {
        ...
        // --- DESIGN curve (φPn / φMn) capped at ACI Pmax ---
    ...
    ///   positive branch ascending (Pmin → Pmax) → top cap at Pmax → negative branch descending (Pmax → Pmin)
        ...
        double pMax, double pMin, CurveAxis axis, bool nominal)
    {
        ...
        // Build loop: positive ascending (Pmin→Pmax) → negative descending (Pmax→Pmin)
        // The junction between positive[last] and negative[first-after-reverse] is the ACI
        // trim cap — a straight horizontal segment at exactly P = pMax.
        ...
        bool forceStraightTrimCap)
    {
        ...
        // Same cap-snap as BuildEnvelopeLoop — guarantee the topmost sample is exactly at pMax.
    ...
    public static IReadOnlyList<string> ValidateStraightTrimSegment(IReadOnlyList<ControlPointDto> points, double designPMaxDisplay)
    {
        ...
        if (atCap.Count != 2)
            defects.Add($"Expected exactly 2 cap points at P≈{designPMaxDisplay:G6}, found {atCap.Count}");
