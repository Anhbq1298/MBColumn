...
<style>html,body{margin:0;padding:0;background:transparent;overflow:hidden;color:{{TEXT_COLOR}};font-family:"Segoe UI Variable Text","Segoe UI","Aptos","Arial",sans-serif}.math-container{display:flex;align-items:center;min-height:{{MIN_HEIGHT}}px;font-size:{{FONT_SIZE}}px;line-height:1.35}.katex{color:{{TEXT_COLOR}};font-size:1em}.katex .mtext,.katex .text{font-family:"Segoe UI Variable Text","Segoe UI","Aptos","Arial",sans-serif}</style>
...
try{katex.render({{LATEX_JSON}},document.getElementById("math"),{throwOnError:false,displayMode:{{DISPLAY_MODE}},strict:"ignore"});report("rendered",{width:Math.ceil(document.body.scrollWidth),height:Math.ceil(document.body.scrollHeight)});}catch(e){report("error",{message:e&&e.message?e.message:"KaTeX render failed"});}
</script></body></html>
""";
    }
