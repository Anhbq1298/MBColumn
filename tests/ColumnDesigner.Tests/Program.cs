...
using ColumnDesigner.Presentation.Wpf.Controls;
...
var tests = new List<(string Name, Action Test)>
{
    ...
    ("ACI trim segment is straight — PMx", TestAciTrimSegmentStraightPmx),
    ("ACI trim segment is straight — PMy", TestAciTrimSegmentStraightPmy),
...
static void TestPmmRatioUsesDesign()
{
    ...
    // A demand near the design boundary should have ratio ≈ 1.0.
    ...
    // RatioCheckService uses PhiPn/PhiMnx/PhiMny — verified in source code review.
}
...
static void Test3DZEqualsActualP()
{
    ...
    // Z is the raw display P value — no offset. Axis origin at engineering P=0.
...
{
    // SelectedPmAngle must be registered with AffectsRender only — no
...
{
    // LC3 inactive — even though its loads are large, it must not be governing.
    var input = MetricInputWithCases(
        ("a", "LC1", 1000, 100, 80, true),
        ("b", "LC2", 2000, 150, 100, true),
...
{
    // No LoadCases → falls back to (Pu, Mux, Muy) from primary fields.
...
static void TestControlPointsTableRowCount()
{
    ...
    // 4 axes × 8 labeled points = 32 rows
